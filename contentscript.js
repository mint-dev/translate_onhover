...
function ignoreThisPage(options) {
  const isBlacklisted =
    $.grep(options.except_urls, function (url) {
      return RegExp(url).test(window.location.href);
    }).length > 0;
  const isWhitelisted =
    $.grep(options.only_urls, function (url) {
      return RegExp(url).test(window.location.href);
    }).length > 0 || options.only_urls.length === 0;
...
function processEvent(e) {
  function getHitWord(e) {
    function restorable(node, do_stuff) {
      ...
      $('transwrapper').replaceWith(
        TransOver.escape_html($('transwrapper').text())
      );
      return res;
    }
    ...
    const text_nodes = hit_elem.contents().filter(function () {
      return (
        this.nodeType == Node.TEXT_NODE && XRegExp(word_re).test(this.nodeValue)
      );
    });
    ...
    const hit_word = restorable(hit_text_node, function () {
      ...
      function getHitText(node, parent_font_style) {
        ...
        if (XRegExp(word_re).test(node.textContent)) {
          $(node).replaceWith(function () {
            return this.textContent.replace(
              ...
              function ($0, $1, $2) {
                return (
                  '<transblock>' +
                  TransOver.escape_html($1) +
                  '</transblock><transblock>' +
                  TransOver.escape_html($2) +
                  '</transblock>'
                );
              }
            );
          ...
          const next_node = document.elementFromPoint(e.clientX, e.clientY)
            .childNodes[0];
  ...
  if (
    /INPUT|TEXTAREA/.test(hit_elem.nodeName) ||
    hit_elem.isContentEditable ||
    $(hit_elem)
      .parents()
      .filter(function () {
  ...
  if (word != '') {
    chrome.extension.sendRequest(
      ...
      function (response) {
        ...
        showPopup(
          e,
          TransOver.formatTranslation(
            translation,
            TransOverLanguages[response.tl].direction,
            response.sl,
            options
          )
        );
...
$(document).click(function (e) {
  withOptionsSatisfied(e, function () {
    if (options.translate_by != 'click') return;
  ...
  .keydown(function (e) {
    if (TransOver.modifierKeys[e.keyCode] == options.popup_show_trigger) {
      ...
      if (options.selection_key_only && selection) {
        ...
        chrome.extension.sendRequest(
          { handler: 'translate', word: selection },
          function (response) {
            debug('response: ', response);
    ...
    if (
      ...
      TransOver.modifierKeys[e.keyCode] == options.tts_key &&
      options.tts &&
      $('transover-popup').length > 0
    ) {
      debug('tts');
...
function hasMouseReallyMoved(e) {
  //or is it a tremor?
  ...
  return (
    e.clientX > right_boundry ||
    e.clientX < left_boundry ||
    e.clientY > bottom_boundry ||
...
window.addEventListener('message', function (e) {
  ...
  if (e.data.type == 'transoverTranslate') {
    chrome.extension.sendRequest(
      { handler: 'translate', word: e.data.text, sl: e.data.sl, tl: e.data.tl },
      function (response) {
